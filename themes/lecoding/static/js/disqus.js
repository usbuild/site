...
class Api {
    ...
    chainArgs(args) {
        return Object.keys(args).map(key => {return `${encodeURIComponent(key)}=${encodeURIComponent(args[key])}`}).join("&")
    }

    get(api, args) {
        return fetch(this.apiUrl + api + ".json?" + this.chainArgs(args))
            .then(resp => {return resp.json()})
    }

    post(api, args) {
        return fetch(this.apiUrl + api + ".json", {
            body: this.chainArgs(args),
            method:"POST",
            headers: {
...
let api = new Api("https://lecoding.com/disqus/");
const forum = "lecoding"
  ...
  class Main extends Jinkela {
    getArticleComments(url) {
      ...
      if (url && url.trim()) query = {forum, "thread":api.threadInfo.id }
      api.get('posts/list', query).then(response => {
  ...
  exports.DisqusJS = {
    ...
    getArticleComments(url = location.href) {
      new Promise((resolve, reject) => {
        ...
        $dsq.src = '//lecoding.disqus.com/embed.js';
        $dsq.onload = () => {
          done = true;
          reject();
        };
